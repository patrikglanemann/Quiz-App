...
import { setCardBookmark } from "/lib/db.js";
...
export function cardCreate(cardData) {
  ...
  const cardButtonDelete = cardButtonDeleteCreate(cardData, section);
...
function cardButtonDeleteCreate(cardData, section) {
  const deleteButton = document.createElement("button");
  deleteButton.classList.add("quiz-card__delete-button");
  deleteButton.textContent = "X";
  deleteButton.addEventListener("click", () => {
    const ask = confirm("Are you sure you want to delete this question card?");
    if (ask === true) {
...
function cardBookmarkCreate(cardData) {
  ...
  cardBookmark.addEventListener("click", () => {
    ...
    const cardID = cardData.id;
    setCardBookmark(cardBookmark.textContent, cardID);
  });
